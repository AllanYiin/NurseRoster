...
## Zeabur 部署策略
- **前後端分離**：前端為單頁式 SPA，雛形已移至 `frontend/public`（index.html + static），後續可替換為 Vite/React build 產物並部署在 Zeabur 靜態站；後端為 FastAPI 容器，提供 `/api/**` 與 SSE/WS 端點。
...
project-root/
├─ backend/
│  └─ app/          # FastAPI 服務，啟動入口 backend/app/__main__.py
├─ frontend/
│  ├─ public/       # SPA 靜態雛形（index.html + static 資源）
│  ├─ src/          # 前端開發原始碼（可替換成 Vite/React）
│  └─ package.json  # 前端啟動入口（scripts.dev）
├─ tests/           # 後端測試（pytest）
...
- `backend/app/core/config.py` 會在啟動時解析根目錄 `.env`（內建解析器），請依 `.env.example` 建立。
...
- 腳本會自動建立 `.venv`、安裝 `requirements.txt`，並以 `python -m app`（工作目錄 `backend/`）啟動服務。
