...
def write_run_app_bat(root: Path, script_name: str, backend: dict,
    ...
    # Backend default host/port if missing
    backend_host = (cfg.get("BACKEND_HOST", "").strip() or backend.get("host") or "127.0.0.1")
    backend_port = safe_int(cfg.get("BACKEND_PORT", "")) or backend.get("port") or 8000
...
start "Backend" cmd /k ""%PYEXE%" -m uvicorn {target} --host {backend_host} --port {backend_port} --log-level info 1>>"logs\backend.log" 2>>&1"
"""
...
if not exist "logs" mkdir "logs"
start "Frontend" cmd /k "cd /d "{frontend.dir}" ^&^& {frontend.install_cmd} 1>>"%~dp0logs\frontend.log" 2>>&1 ^&^& {frontend.run_cmd} 1>>"%~dp0logs\frontend.log" 2>>&1"
"""
        # 若偵測不到 port，就不開（避免亂開）
        if frontend.port:
...
def full_auto(root: Path, venv_dir: str) -> Tuple[int, str]:
    ...
    if backend.get("mode") == "uvicorn":
        ...
        host = cfg.get("BACKEND_HOST", "").strip() or backend.get("host") or "127.0.0.1"
        port = safe_int(cfg.get("BACKEND_PORT", "")) or backend.get("port") or 8000
